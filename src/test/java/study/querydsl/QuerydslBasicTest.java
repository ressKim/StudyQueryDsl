    ...
    public void findDtoByConstructor() {
        List<UserDto> result = queryFactory
                .select(Projections.constructor(UserDto.class,
                        member.username,
                        member.age))//여기서 값이 더 추가하면 컴파일에러는 못잡고 런타임 에러가 발생한다
                .from(member)
                .fetch();
...
//        }
    ...
    public void findDtoByQueryProjection() {
        List<MemberDto> result = queryFactory
                //여기는 값 추가될 경우 컴파일 오류를 발생시켜서 빨리 잡을 수 있다. command+p 하면 타입도 볼수있다.
                //단점 - QDto 생성해야됨, 의존관계 문제가 좀 있는데 이건 상황따라 판단해 보자
    ...
    //동적쿼리 처리하는 방식 - BooleanBuilder or Where 다중 파라미터 방식
    // - BooleanBuilder
    @Test
...
//                .where(allEq(usernameCond, ageCond))
    ...
    //광고 상태 isValid, 날짜가 IN : isServiceable 이런식으로 조립으로 만들 수 있다. -
    // ** 재사용 가능
    ...
    public void bulkUpdate() {
        ...
        //벌크연산을 할때는 영속성 컨텍스트 상태를 무시하고 바로 db 에 넣는거라서 조심해야된다.
        ...
        em.flush();
        em.clear();//벌크연산이라서 영속성 컨텍스트를 초기화 해주자.
